/*==============================================================*/
/* DBMS name:      MySQL 5.0                                    */
/* Created on:     6/8/2017 2:19:29 PM                          */
/*==============================================================*/
...
(
   DATA_ID              BIGINT NOT NULL AUTO_INCREMENT COMMENT '数据主键ID',
   ...
   USETYPE              SMALLINT NOT NULL DEFAULT 0 COMMENT '用它类型[ 历史数据(0) | 预测数据(1)]',
...
CREATE TABLE AI_JOB
(
   JOB_ID               BIGINT NOT NULL AUTO_INCREMENT COMMENT 'Job主键ID',
   ...
   JOB_DATA             TEXT NOT NULL COMMENT 'Job 结果',
   LATEST_DATE          DATETIME COMMENT '最后一次操作日期',
   CREATE_DATE          DATETIME COMMENT '创建日期',
   OPERATOR             VARCHAR(32) COMMENT '操作人',
   DATA_STATUS          SMALLINT DEFAULT 1 COMMENT '数据状态[有效(1)|禁用(0)|已删除(-1)]',
...
(
   MODEL_ID             BIGINT NOT NULL AUTO_INCREMENT COMMENT '模型主键ID',
   MODEL_NAME           VARCHAR(64) NOT NULL COMMENT '模型名称',
   MODEL_DESC           TEXT NOT NULL COMMENT '模型说明',
   MODEL_PARAMS         TEXT NOT NULL COMMENT '模型参数说明',
   LATEST_DATE          DATETIME COMMENT '最后一次操作日期',
   CREATE_DATE          DATETIME COMMENT '创建日期',
   OPERATOR             VARCHAR(32) COMMENT '操作人',
   DATA_STATUS          SMALLINT DEFAULT 1 COMMENT '数据状态[有效(1)|禁用(0)|已删除(-1)]',
...
(
   ...
   FILE_CONFIG          TEXT NOT NULL COMMENT '数据规格说明',
...
ALTER TABLE AI_JOB ADD CONSTRAINT FK_TASK_JOB FOREIGN KEY (TASK_ID)
      REFERENCES AI_TASK (TASK_ID) ON DELETE RESTRICT ON UPDATE RESTRICT;

ALTER TABLE AI_MODEL_VERSION ADD CONSTRAINT FK_MODEL_VERSION FOREIGN KEY (MODEL_ID)
      REFERENCES AI_MODEL (MODEL_ID) ON DELETE RESTRICT ON UPDATE RESTRICT;

ALTER TABLE AI_TASK ADD CONSTRAINT FK_TASK_MODEL FOREIGN KEY (MODEL_ID)
      REFERENCES AI_MODEL (MODEL_ID) ON DELETE RESTRICT ON UPDATE RESTRICT;

ALTER TABLE AI_TASK_DATA ADD CONSTRAINT FK_TASK_DATA FOREIGN KEY (TASK_ID)
      REFERENCES AI_TASK (TASK_ID) ON DELETE RESTRICT ON UPDATE RESTRICT;

ALTER TABLE AI_TASK_DATA ADD CONSTRAINT FK_TASK_DATA2 FOREIGN KEY (DATA_ID)
      REFERENCES AI_DATA_FILE (DATA_ID) ON DELETE RESTRICT ON UPDATE RESTRICT;
